...
export default function App() {
  ...
  const connectWebSocket = () => {
    try {
      ...
      wsRef.current.onopen = () => {
        ...
        showToast("Connected to live leaderboard ranking", "success");
      };
  ...
  const startLeaderboardPolling = () => {
    ...
    showToast("Server offline. Switched to REST live polling.", "info");
  ...
  const handleStartOnboarding = async (e: React.FormEvent) => {
    ...
    if (!cleanedName) {
      showToast("Name is mandatory to start!", "error");
      return;
    }
    if (cleanedName.length > 30) {
      showToast("Name must be under 30 characters!", "error");
      return;
    }
  ...
  const initializeSession = (id: string, name: string, isOffline = false) => {
    ...
    if (isOffline) {
      showToast("Connection offline. Running in Local Offline Mode!", "info");
    } else {
      showToast(`Welcome ${name}! Go find those bugs!`, "success");
    }
  ...
  const handleBugTap = (bugId: string) => {
    if (bugsFound.includes(bugId)) {
      showToast("You have already reported this bug!", "error");
      return;
    }
    ...
    if (bugInfo) {
      ...
      setModalTitle(bugInfo.titleDefault);
      setModalDescription(bugInfo.descriptionDefault);
      setModalError('');
    }
  };
  ...
  const handleBugSubmit = async (e: React.FormEvent) => {
    ...
    if (modalDescription.trim().length < 10) {
    ...
    if (offlineMode) {
      ...
      setScore(updatedBugs.length);
      localStorage.setItem('moto_bugs_found', JSON.stringify(updatedBugs));
      
      showToast("Bug reported locally! +1 Point", "success");
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast("Bug successfully verified! +1 Point", "success");
        setReportingBugId(null);
      } else {
        const errorData = await response.json();
        showToast(errorData.detail || "Failed to submit report.", "error");
      }
    } catch (err) {
      showToast("Submission failed. Switched to offline saving.", "error");
  ...
  const handleGameOver = async (id: string, finalScore: number) => {
    ...
    if (!offlineMode) {
      try {
        ...
        logger("Failed to sync session end to API");
      }
    }
  };
  ...
  const handleCartQtyIncrease = (productId: string) => {
    if (productId === 'moto_g_power') {
      ...
      showToast("Quantity reset to 0 due to critical cart script exception!", "error");
    } else {
      ...
      <div className="min-h-screen bg-[#06050b] text-white flex flex-col justify-between p-8 relative overflow-hidden font-sans select-none">
              ...
              {wsConnected ? (
                ...
                <>
                  ...
                  <span className="text-xs font-semibold text-amber-400 uppercase tracking-widest flex items-center gap-1">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" /> REST Polling
                  </span>
                </>
              )}
            </div>
        ...
        <div className="flex-1 my-8 flex flex-col justify-center items-center z-10">
          <div className="w-full max-w-5xl bg-slate-900/30 rounded-3xl border border-white/5 backdrop-blur-xl shadow-2xl p-6 relative">
            ...
            <div className="grid grid-cols-12 text-sm font-semibold tracking-wider text-slate-400 uppercase border-b border-white/5 pb-4 px-6 mb-2">
              ...
              <div className="col-span-2 text-right">Bugs Found</div>
            </div>

            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
              {leaderboard.length === 0 ? (
                <div className="text-center py-20 text-slate-500 font-medium">
                  Waiting for players to submit bugs... Scan QR Code on mobile to start!
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-2 text-center text-sm font-mono text-slate-400">
                        {player.created_at ? new Date(player.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Live"}
                      </div>
        ...
        <div className="flex justify-between items-center border-t border-white/5 pt-6 z-10">
          <p className="text-sm text-slate-500 font-medium">
            © 2026 Motorola Mobile Challenge. Designed exclusively for Developer Booth Sync.
          </p>
          <div className="flex items-center gap-4 bg-slate-900/60 px-5 py-3 rounded-2xl border border-white/10 shadow-lg">
            <div className="text-right">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Join the Game</p>
              <p className="text-sm font-extrabold text-cyan-400">Scan QR Code at the Counter!</p>
            </div>
          ...
          <div className="flex-1 flex flex-col justify-between p-6">
            <div className="my-auto space-y-8 py-10">
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-white/5 p-4.5 space-y-3.5 shadow-xl">
                <h3 className="text-sm font-extrabold text-violet-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-violet-400" /> Challenge Rules:
                </h3>
                <ul className="text-xs text-slate-300 space-y-2.5">
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 text-cyan-400 font-bold">1.</span>
                    <span>You have <strong>2 minutes (02:00)</strong> to explore a simulated Motorola e-commerce catalog.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 text-cyan-400 font-bold">2.</span>
                    <span>Find intentional bugs (e.g. price bugs, broken layouts, navigation loop freezes).</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 text-cyan-400 font-bold">3.</span>
                    <span>Tap directly on the broken elements, report them, and claim your place on the big TV board!</span>
                  </li>
                </ul>
              </div>
              ...
              <form onSubmit={handleStartOnboarding} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    required
                    maxLength={30}
                    placeholder="e.g. Diego (QA Expert)"
                    value={playerName}
                ...
                >
                  <Play className="w-4 h-4 fill-current text-white" /> Start Challenge
                </button>
              </form>
            </div>
            ...
            <div className="text-center pt-4">
              <p className="text-[10px] text-slate-500 font-bold tracking-widest uppercase flex items-center justify-center gap-1.5">
                <Smartphone className="w-3.5 h-3.5 text-slate-500" /> MOBILE OPTIMIZED
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 flex flex-col">
            ...
            <header className="sticky top-0 bg-[#0d0c15]/95 border-b border-white/5 px-4 py-3.5 backdrop-blur-md z-30 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h1 className="text-xs font-black tracking-tight leading-none text-white">MOTO SIMULATION</h1>
                  <span className="text-[9px] font-bold text-cyan-400 leading-none tracking-widest uppercase">CIN-DAY</span>
                </div>
              </div>
            ...
            <div className="p-4">
              <div className="bg-gradient-to-r from-violet-900/50 via-indigo-900/30 to-cyan-950/40 rounded-2xl p-4 border border-white/5 relative overflow-hidden">
                ...
                <h3 className="text-sm font-black tracking-tight text-white mb-1 uppercase">EXCLUSIVE BOOTH SALE</h3>
                <p className="text-[11px] text-slate-300 leading-relaxed max-w-[80%] mb-2">
                  Find hidden code discrepancies in this live checkout process. Test carefully!
                </p>
            ...
            <div className="flex-1 px-4 py-2">
              {filteredProducts.length === 0 ? (
                // BUG E: accessory selection triggers blank screen
                <div 
                  onClick={() => handleBugTap('bug_filter')}
                  className="min-h-[250px] rounded-2xl border border-dashed border-red-500/25 bg-red-550/5 flex flex-col items-center justify-center p-6 text-center cursor-pointer relative"
                >
                  {/* Subtle hover bug target overlay */}
                  <div className="absolute top-2 right-2 p-1 bg-red-500/10 rounded-lg text-red-400 opacity-60">
                    <BugIcon className="w-3.5 h-3.5" />
                  </div>
                  <AlertTriangle className="w-8 h-8 text-red-500/55 mb-2" />
                  <p className="text-xs font-semibold text-red-400 uppercase tracking-widest mb-1">
                    Blank Screen Filter Bug Detected
                  </p>
                  <p className="text-[10px] text-slate-500">
                    This selection broke the page completely. Tap here to Report!
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <div className="text-[10px] text-slate-400 line-clamp-2 leading-relaxed min-h-[30px]">
                            {isBugText ? (
                              <div 
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="border border-dashed border-red-500/30 bg-red-500/5 p-1 rounded cursor-pointer relative"
                              >
                                <span className="absolute top-0.5 right-0.5 text-[7px] text-red-400 font-extrabold uppercase bg-red-900/50 px-1 rounded-sm">Bug</span>
                          ...
                          <div className="flex justify-between items-end">
                            {isBugPrice ? (
                              <div 
                                ...
                                className="border border-dashed border-red-500/30 bg-red-500/5 px-2 py-0.5 rounded cursor-pointer relative w-full"
                              >
                                <span className="absolute top-0.5 right-1 text-[7px] text-red-400 font-bold uppercase bg-red-900/50 px-1 rounded-sm">Bug</span>
                                <span className="text-xs text-slate-500 line-through block font-mono">$299.99</span>
                                <span className="text-sm font-black text-rose-400 font-mono block">-$999.00</span>
                              </div>
                            ) : (
                              <div>
                                {prod.originalPrice && (
            ...
            <div className="sticky bottom-0 inset-x-0 p-4 bg-[#0a0910]/95 border-t border-white/5 backdrop-blur-md z-20 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm font-black text-white">{cartItemCount} Products</p>
              </div>
              <button
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {Object.keys(cart).length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center p-6">
                        ...
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Your cart is empty</p>
                        <p className="text-[10px] text-slate-500">Go back and add some products!</p>
                      </div>
                    ) : (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {isBugLoop ? (
                                <div 
                                  ...
                                  className="border border-dashed border-red-500/40 bg-red-500/5 px-2 py-0.5 rounded cursor-pointer relative font-mono text-xs font-extrabold text-red-400"
                                >
                                  <span className="absolute top-[-5px] right-[-5px] w-2 h-2 rounded-full bg-red-500 animate-ping" />
                                  {qty}
                                </div>
                              ) : (
                  ...
                  <div className="p-4 border-t border-white/5 bg-slate-900/30 space-y-4">
                    ...
                    <div className="relative">
                      ...
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Shipping</span>
                      ...
                      <div 
                        ...
                        className="absolute inset-0 bg-red-500/10 border border-dashed border-red-500/35 rounded-lg flex items-center justify-center text-[10px] font-black tracking-widest text-red-400 cursor-pointer hover:bg-red-500/15 transition-all z-10 opacity-0 hover:opacity-100"
                        title="Tapping on this overlap lets you report Bug B (Overlapping checkout layouts)!"
                      >
                        <BugIcon className="w-4 h-4 mr-1 animate-spin" /> REPORT OVERLAP BUG
                      </div>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (cartTotal < 0) {
                          showToast("Invalid checkout: Price can't be negative. Report Bug A!", "error");
                        } else {
                          showToast("Simulation Complete! Standard mock checkout completed.", "success");
                          setIsCartOpen(false);
                        }
                      }}
                      ...
                      style={{ marginTop: '-8px' }} // Seed B: Overlaps total above!
                    >
                      Secure Checkout
                    </button>
                  </div>
              ...
              <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
                <div className="w-full max-w-sm bg-slate-900 rounded-3xl border border-red-500/30 shadow-2xl overflow-hidden relative animate-bounce-in">
                  ...
                  <div className="p-5 border-b border-white/5 flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-extrabold text-white uppercase tracking-wider">Report Seeded Bug</h4>
                      <p className="text-[10px] text-slate-500 font-bold">Verification: {reportingBugId}</p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleBugSubmit} className="p-5 space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        placeholder="Explain how to reproduce the bug and what went wrong."
                      />
                    </div>
          ...
          <div className="flex-1 flex flex-col justify-between p-6 text-center">
            ...
            <div className="my-auto space-y-8 py-10">
              ...
              <div className="bg-slate-900/60 rounded-3xl border border-white/5 p-6 space-y-3.5 shadow-2xl relative">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Player Profile</p>
                  <p className="text-xl font-extrabold text-white tracking-wide uppercase">{playerName}</p>
                </div>

                <div className="pt-2">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Bugs Identified Successfully</p>
                  <div className="flex justify-center items-end gap-1.5 font-mono">
                    <span className="text-5xl font-black text-transparent bg-gradient-to-r from-cyan-400 to-violet-400 bg-clip-text">
                      {score}
                    </span>
                    <span className="text-lg font-bold text-slate-600 mb-1">/ 5</span>
                  </div>
                </div>

                {/* Rank Feedback */}
                <p className="text-xs font-semibold text-slate-300 leading-relaxed px-4">
                  {score === 5 ? "🏆 Absolute Legend! Outstanding work, you found every single error!" :
                   score >= 3 ? "👏 Amazing QA skills! You captured most of our developer's mistakes!" :
                   score >= 1 ? "👍 Good effort! You successfully captured some tricky glitches!" :
                   "🧐 Keep practicing! Testing requires extreme precision and patience."}
                </p>
              </div>
              ...
              <div className="space-y-3.5 pt-4">
                ...
                >
                  <RefreshCw className="w-4 h-4 text-slate-400" /> Start New Session
                </button>
                
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide px-6">
                  Check the live booth display ranking to see where your score landed!
                </p>
              </div>
            </div>
